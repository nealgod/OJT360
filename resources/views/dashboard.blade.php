        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(Auth::user()->isStudent())
                ...
                @if(Auth::user()->isStudent())
                    @php
                        $hasResume = false;
                        if (class_exists('\App\Models\Resume')) {
                            try {
                                $hasResume = \App\Models\Resume::where('user_id', Auth::id())->exists();
                            } catch (\Exception $e) {
                                $hasResume = false;
                            }
                        }
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-ojt-dark mb-2">Resume Builder</h3>
                                <p class="text-gray-600 text-sm">Create and manage your professional resume using our builder.</p>
                            </div>
                            <a href="{{ route('resume.index') }}" class="inline-flex items-center px-4 py-2 {{ $hasResume ? 'bg-white border border-ojt-primary text-ojt-primary hover:bg-ojt-primary hover:text-white' : 'bg-ojt-primary text-white hover:bg-maroon-700' }} text-sm font-medium rounded-lg transition-colors">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
