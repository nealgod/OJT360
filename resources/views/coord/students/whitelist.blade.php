			...
			<div class="bg-white rounded-lg border border-gray-200 p-6">
				<div class="flex items-center justify-between mb-4">
					...
					<div class="flex space-x-2">
						<a href="{{ route('coord.students.import') }}" class="px-4 py-2 bg-ojt-primary text-white rounded">Upload Class List</a>
						<form method="POST" action="{{ route('coord.students.whitelist.end-term') }}" onsubmit="return confirm('Archive all current pending and activated records?')">
							@csrf
							<button type="submit" class="px-4 py-2 bg-gray-800 text-white rounded">End Term: Archive All</button>
						</form>
					</div>
				</div>
