<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('student-documents.resume.store') }}" id="resumeForm" enctype="multipart/form-data" class="space-y-6">
                @csrf
                            ...
                            <div>
                                ...
                                <select name="education[0][year_level]" 
                                    ...
                                    @foreach(($yearLevels ?? []) as $code => $label)
                                        <option value="{{ $label }}" {{ old('education.0.year_level', $defaultData['education'][0]['year_level'] ?? '') === $label ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
