<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('resume.update', $resume) }}" id="resumeForm" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div class="bg-white shadow sm:rounded-lg p-6">
                    ...
                    <textarea id="objective" name="objective" rows="4"
                              class="w-full rounded-lg border-gray-300 focus:border-ojt-primary focus:ring-ojt-primary"
                              placeholder="Write a brief summary of your career objectives or professional summary...">{{ old('objective', $resume->objective ?? '') }}</textarea>
                    @error('objective')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <!-- Education -->
                <div class="bg-white shadow sm:rounded-lg p-6" x-data="{ educations: {{ json_encode(old('education', $resume->education ?? [['institution' => '', 'degree' => '', 'department' => '', 'year' => '']])) }} }">
