    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Update company information</p>
            </div>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <form method="POST" action="{{ $isAdmin ? route('admin.companies.update', $company) : route('coord.companies.update', $company) }}" class="p-6 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="status" 
                            ...
                            <option value="active" {{ old('status', $company->status) === 'active' ? 'selected' : '' }}>Active - Available for students</option>
                            <option value="inactive" {{ old('status', $company->status) === 'inactive' ? 'selected' : '' }}>Inactive - Not available</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-2 text-sm text-gray-500">Active companies will be visible to students in the assigned department</p>
                    </div>
                ...
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-blue-800">
                        ...
                        <ul class="list-disc list-inside space-y-1">
                            @if($isAdmin)
                                <li>Department can be changed by admin</li>
                            @else
                                <li>Department: <strong>{{ $company->department }}</strong> (cannot be changed)</li>
                            @endif
