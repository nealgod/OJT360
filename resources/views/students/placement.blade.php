                        ...
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <p class="text-sm text-gray-600 flex items-start">
                                    ...
                                    <span>{{ $company?->address ?? $externalCompanyAddress ?? 'Address not set' }}</span>
                                </p>
                                @if($placement?->start_date || $acceptance?->start_date)
                                    <p class="text-sm text-gray-600 flex items-center">
                                        <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                        </svg>
                                        Started {{ ($acceptance?->start_date ?? $placement?->start_date)->format('M d, Y') }}
                                    </p>
                                @endif
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-700">
                        <div class="space-y-3">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500 mb-1">Shift Hours</p>
                                <p class="text-base font-medium">{{ $placement->shift_start }} – {{ $placement->shift_end }}</p>
                            </div>
                        </div>
                        <div>
                            <p class="text-xs uppercase tracking-wide text-gray-500 mb-1">Working Days</p>
                            <div class="flex flex-wrap gap-2 mt-2">
                                @if($placement->working_days)
                                    @foreach($placement->working_days as $day)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-ojt-primary/10 text-ojt-primary">
                                            {{ ucfirst($day) }}
                                        </span>
                                    @endforeach
