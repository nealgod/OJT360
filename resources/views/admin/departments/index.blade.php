    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Manage academic departments and their programs</p>
            </div>
            <button onclick="document.getElementById('addDeptModal').classList.remove('hidden')" class="bg-ojt-primary text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-maroon-700 transition-colors shadow-sm flex items-center justify-center gap-2 text-sm sm:text-base">
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($errors->any())
                <div class="bg-red-50 border-l-4 border-red-400 p-4 mb-6 rounded-r-lg shadow-sm">
                    <div class="flex">
                        ...
                        <div class="flex-1">
                            <p class="text-red-700 font-medium mb-2 text-sm sm:text-base">Please fix the following errors:</p>
            ...
            <div class="space-y-6">
                        ...
                        <div class="p-4 sm:p-6">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        <div class="w-10 h-10 sm:w-12 sm:h-12 bg-ojt-primary bg-opacity-10 rounded-lg flex items-center justify-center flex-shrink-0">
                                            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-ojt-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                            </svg>
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <h3 class="text-lg sm:text-xl font-bold text-ojt-dark truncate">{{ $dept->name }}</h3>
                                            @if($dept->slug)
                                                <p class="text-xs sm:text-sm text-gray-500">Slug: <span class="font-medium">{{ $dept->slug }}</span></p>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div class="flex flex-row sm:flex-col lg:flex-row gap-2 flex-wrap sm:flex-nowrap">
                                    <button onclick="openEditDept({{ $dept->id }}, '{{ addslashes($dept->name) }}', '{{ $dept->slug }}')" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors whitespace-nowrap">
                                        Edit
                                    </button>
                                    <button onclick="openAddProgram({{ $dept->id }}, '{{ addslashes($dept->name) }}')" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-green-600 hover:bg-green-50 rounded-lg transition-colors whitespace-nowrap">
                                        Add Program
                                    </button>
                                    <form action="{{ route('admin.departments.destroy', $dept) }}" method="POST" class="inline flex-1 sm:flex-none" onsubmit="return confirm('Are you sure you want to delete this department? This action cannot be undone.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-full px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors whitespace-nowrap">
                                            Delete
                                        </button>
                                    </form>
                                </div>
                            ...
                            @if($dept->programs->count() > 0)
                                <div class="border-t pt-4 mt-4">
                                    <h4 class="font-semibold text-gray-700 mb-3 flex items-center text-sm sm:text-base">
                                            ...
                                            <div class="bg-gray-50 border border-gray-200 p-3 sm:p-4 rounded-lg hover:bg-gray-100 transition-colors">
                                                <div class="flex justify-between items-start gap-2">
                                                    <div class="flex-1 min-w-0">
                                                        <p class="font-semibold text-gray-900 text-sm sm:text-base truncate">{{ $program->name }}</p>
                                                    ...
                                                    <div class="flex gap-1 flex-shrink-0">
                                                        <button onclick="openEditProgram({{ $program->id }}, '{{ addslashes($program->name) }}', '{{ $program->slug }}', {{ $program->required_hours ?? 0 }})" class="p-1.5 sm:p-2 text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Edit">
                                                        ...
                                                        <form action="{{ route('admin.programs.destroy', $program) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this program?')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="p-1.5 sm:p-2 text-red-600 hover:bg-red-50 rounded transition-colors" title="Delete">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @empty
                    <div class="bg-white rounded-lg border p-8 sm:p-12 text-center">
                        <svg class="mx-auto h-12 w-12 sm:h-16 sm:w-16 text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                        </svg>
                        <p class="text-gray-500 text-base sm:text-lg font-medium">No departments found</p>
